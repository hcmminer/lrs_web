...
export class ListBtsComponent implements OnInit, AfterViewInit {
  ...
  provinceObj = new BehaviorSubject<any>({});
  ...
  ngOnInit(): void {
    this.getComboBoxDataBTSStation().subscribe(res => {
      ...
      this.getListDistrict(localStorage.getItem('user-province')).subscribe(res => {
  ...
  startEdit(item): void {
    ...
    if (localStorage.getItem('user-province')) {
      this.getListDistrict(localStorage.getItem('user-province')).subscribe(res => {
      ...
      if (res.errorCode == '0') {
        ...
        this.listStations.value.map(item => {
          ...
          if (item.hasElectricity || item.hasElectricity == 0) {
            let electricObj = this.listYesOrNoToUpdate.find(type => type.value == item.hasElectricity);
  ...
  conditionSearch() {
    const requestTarget = {
      ...
      btsStationDTO: {
        ...
        hasCRFile: this.searchForm.get('CRStation').value,
        hasContractFile: this.searchForm.get('contractedStation').value
      }
    };
