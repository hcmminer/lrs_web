...
export class AnnualReportComponent implements OnInit, OnDestroy {
  ...
  searchReport() {
    ...
    const sb = this.rpService.searchReport(params, CONFIG.API_PATH.REPORT_STATION_ANNUAL_HTML)
      .pipe(
        first(),
  ...
  exportReport() {
    ...
    const sb = this.rpService.exportReport(params, CONFIG.API_PATH.REPORT_STATION_MONTHLY_FILE)
      ...
      .subscribe((res) => {
        this.isLoading.next(false);
        this.rpService.saveFile(`reportFileYearly${params.p_from_date}-${params.p_to_date}.xlsx`, res);
