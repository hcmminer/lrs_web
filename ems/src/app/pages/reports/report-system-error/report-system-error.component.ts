...
@Component({
  selector: 'app-report-system-error',
  templateUrl: './report-system-error.component.html',
  styleUrls: ['./report-system-error.component.scss']
})
export class ReportSystemErrorComponent implements OnInit , OnDestroy {
  query = {
    ...queryInit
  };
  ...
  searchReportSystemError() {
    ...
    const sb = this.reportService.searchReport(params, CONFIG.API_PATH.REPORT_SYSTEM_ERROR_HTML).pipe(
      first(),
  ...
  exportReportSystemError() {
    ...
    const sb = this.reportService.exportReport(params, CONFIG.API_PATH.REPORT_SYSTEM_ERROR_FILE).pipe(
      first(),
      catchError((errorMessage) => {
        this.modal.dismiss(errorMessage);
        return of({
      ...
      this.reportService.downExportExcel(
        res && res.data,
        `report_system_error_${fileTime}.xls`
      );
    });
    this.subscriptions.push(sb);
  }
