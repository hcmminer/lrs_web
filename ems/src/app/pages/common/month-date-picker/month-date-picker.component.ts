import { Component, forwardRef, HostBinding, Input, ViewChild, EventEmitter, Output, ElementRef } from '@angular/core';
...
export class MonthDatePickerComponent implements ControlValueAccessor {
  ...
  @Output() getDataTxt = new EventEmitter<string>();
  ...
  change(value: string) {
    if(!/^[\d-]*$/g.test(value)){
      this.dataTxt = this.previousDataTxt;
      this.inputText.nativeElement.value = this.previousDataTxt;
      this.getDataTxt.emit(this.dataTxt);
      return;
    }
    ...
    this.dataTxt = value;
    this.previousDataTxt = this.dataTxt;
  ...
  onChange = (data: Idata) => {
    this.data = data;
    this.dataTxt = this.monthFirst ? "" + data.month + this.separator + data.year : "" + data.year + this.separator + data.month;
    this.previousDataTxt = this.dataTxt;
    this.incr=this.getIncr(this.data.year);
  };
