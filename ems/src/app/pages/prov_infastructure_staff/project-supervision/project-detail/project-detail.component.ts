...
@Injectable()
export class ProjectDetailDBTablePIS {
...
@Component({
  ...
  providers: [ProjectDetailDBTablePIS]
})
export class ProjectDetailComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    private database: ProjectDetailDBTablePIS,
    ...
    sessionStorage.removeItem('project_detail_tree_infas');
  ...
  checkApproveParent(item) {
    if (item && item.listItemDetailDTO && item.listItemDetailDTO.length > 0){
      if (item.listItemDetailDTO.some(child => child.status !== 3)) {
        return false;
      }
    }
    return true;
  ...
  saveCacheExpandNode(){
    sessionStorage.setItem('project_detail_tree_infas', JSON.stringify(this.treeControl.dataNodes));
  }
  getNodeExpand(): TreeConstructDetailDTOFlat[]{
    let expands = [];
    const dataTreeInStorage = JSON.parse(sessionStorage.getItem('project_detail_tree_infas'));
