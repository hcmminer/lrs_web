...
export class WebsocketServiceGroup {
  stompClient: any;
  ws: any;
  ...
  onMessageReceived(message) {
    ...
    if (indexSer > -1 && newMess.clearStatus !== '1') {
    ...
    const checkDup = new BehaviorSubject<number>(-1);
    this.dashboardOnlineService.listDashboard.value.forEach((val, index) => {
      if (val.faultId.toString() === newMess.faultId.toString()) {
        checkDup.next(index);
        // val = newMess;
      }
    });
    checkDup.subscribe(value => {
      if (value > -1) {
        this.dashboardOnlineService.listDashboard.value[value] = newMess;
        // this.dashboardOnlineService.listDashboard.next(this.dashboardOnlineService.listDashboard.value);
      } else {
        this.dashboardOnlineService.listDashboard.value.unshift(newMess);
    ...
    this.dashboardOnlineService.listDashboard.next(this.dashboardOnlineService.listDashboard.value);
    // const inF = this.dashboardOnlineService.listDashboard.value.findIndex(val => val.faultId.toString() === newMess.faultId.toString());
    // if(inF > -1){
    //   this.dashboardOnlineService.listDashboard.value[inF] = newMess;
    // }else{
    //   this.dashboardOnlineService.listDashboard.value.unshift(newMess);
    // }
    ...
    //   const index = listTmp.findIndex(value1 => value1.alarmId.toString() === value.alarmId.toString());
    //   if (index === -1) {
    //     listTmp.push(value);
    //   }
    // });
    // this.dashboardOnlineService.listDashboard.next(listTmp);
    console.log(this.dashboardOnlineService.listDashboard);
    this.dashboardOnlineService.paginatorState.next({
      ...
      total: this.dashboardOnlineService.listDashboard.value.length
    } as PaginatorState);
    // Emits the event.
  ...
  playAudio() {
    if (this.dashboardOnlineService.isPlayAudio) {
      if (this.dashboardOnlineService.listDashboard.value[0] &&
          this.dashboardOnlineService.listDashboard.value[0].alarmSeverityId) {
