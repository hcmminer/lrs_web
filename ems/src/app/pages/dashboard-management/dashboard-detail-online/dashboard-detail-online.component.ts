...
import {BehaviorSubject, of, Subscription, fromEvent, Observable} from 'rxjs';
...
@Component({
  selector: 'app-dashboard-detail-online',
  templateUrl: './dashboard-detail-online.component.html',
  styleUrls: ['./dashboard-detail-online.component.scss']
})
export class DashboardDetailOnlineComponent implements OnInit, AfterViewInit, OnDestroy {
  @ViewChild('vcSer') vSer: ElementRef;
  ...
  checkF1 = new BehaviorSubject<number>(0);
  listType = [];
  listSeverity = [];
  ...
  ngOnInit(): void {
    ...
    this.dashboardOnlineService.listDashboardDetail.next([]);
    // this.ePlay();
    // Object.assign(this.query,{serviceCode: this.ser});
    ...
    this.checkF.subscribe(value => {
      if (value === 3) {
        ...
        this.query.serviceCode = this.dashboardOnlineService.serviceCode;
        this.dashboardOnlineService.getListDashboardDetail(this.query).subscribe((res: ResponseModel) => {
          if (!res.status) {
            throw new Error(res.message);
          } else {
            this.dashboardOnlineService.listDashboardDetail.next([]);
            if (typeof (res.data) !== 'undefined' && res.data !== null && res.data.toString().length > 0) {
              this.dashboardOnlineService.listDashboardDetail.next(res.data as DashboardDetailModel[]);
            }
            this.dashboardOnlineService.paginatorState.next({
              page: res.currentPage,
              pageSize: res.pageLimit,
              total: this.dashboardOnlineService.listDashboardDetail.value ?
                  this.dashboardOnlineService.listDashboardDetail.value.length : 0
            } as PaginatorState);
            this.dashboardOnlineService.serviceCode = '';
            this.setColorInRow();
          }
        });
      }

      this.checkF1.subscribe(value1 => {
        // if (value1 > 1){
        this.loadSearchForm();
        // }
      });
    ...
    const lstSer = this.dashboardOnlineService.getListServiceBox({
      ...
      if (!res.status) {
        ...
        // });
        // this.query.serviceCode = null;

        // this.query.serviceCode = this.ser;
      }
    });
  ...
  ePlay() {
    if (!this.dashboardOnlineService.isPlayAudio) {
      ...
      if (this.dashboardOnlineService.listDashboardDetail.value[0] && this.dashboardOnlineService.listDashboardDetail.value[0].alarmSeverityId) {
      ...
      if (!this.isValidDate(iValidStartDateStr)) {
        ...
        if (new Date(this.query.iValidStartDate['year'], this.query.iValidStartDate['month'] - 1, this.query.iValidStartDate['day']) > currentDate) {
          this.startDateErrorMsg = this.translateService.instant('VALIDATION.AFTER_CURRENT_DATE',
              {name: this.translateService.instant('ALARM_DASHBOARD.ALARM_DASHBOARD_DETAIL.TABLE_TITLE.START_DATETIME')});
          isValidAllDate = false;
        }
      }
    }
