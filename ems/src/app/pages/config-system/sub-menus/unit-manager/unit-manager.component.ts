...
import { AddEditUnitComponent } from './childs/add-edit-unit/add-edit-unit.component';
...
export class UnitManagerComponent implements OnInit, OnDestroy {
  ...
  openModal(item, action) {
    const modalRef = this.modalService.open(AddEditUnitComponent, {
      centered: true,
      backdrop: false,
      size: 'xl',
    });
  ...
  conditionSearch() {
    const requestTarget = {
      functionName: 'listUnit',
      searchV1DTO: {
        provinceId: this.searchForm.get('provinceId').value,
        unitName: this.searchForm.get('unitName').value,
      },
    };
