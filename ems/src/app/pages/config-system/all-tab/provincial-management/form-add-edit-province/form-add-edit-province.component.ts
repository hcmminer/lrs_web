...
@Component({
  selector: "app-form-add-edit-province",
  templateUrl: "./form-add-edit-province.component.html",
  styleUrls: ["./form-add-edit-province.component.scss"],
})
export class FormAddEditProvinceComponent implements OnInit {
  //biến
  req;
  isUpdate;
  isUpdateFile;
  ...
  loadAddForm() {
    this.addEditForm = this.fb.group({
      provinceCode: [
        this.isUpdate ? this.item.proCode : "",
        [Validators.required],
      ],
      provinceName: [
        this.isUpdate ? this.item.proName : "",
        [Validators.required],
      ],
    });
  }
  ...
  conditionAddEdit() {
    const requestTarget = {
      functionName: this.isUpdate ? 'updateProvince' : 'addProvince',
      userName: this.userName,
      provinceDTO: {
        proId: this.isUpdate ? this.item.proId : null,
        proName: this.addEditForm.get("provinceName").value,
        proCode: this.addEditForm.get("provinceCode").value,
      },
    };
    return this.provincialManagementService.callAPICommon(requestTarget as RequestApiModel);
  }
  ...
  save() {
    ...
    modalRef.componentInstance.data = {
      ...
      message: this.isUpdate
        ? this.translate.instant("PROVINCIAL_MANAGEMENT.CONFIRM_UPDATE")
        : this.translate.instant("PROVINCIAL_MANAGEMENT.CONFIRM_ADD"),
      continue: true,
      cancel: true,
      btn: [
        {
        ...
        let request = this.conditionAddEdit().subscribe((res) => {
          if (res.errorCode === "0") {
            this.toastrService.success(
              this.isUpdate
                ? this.translate.instant("PROVINCIAL_MANAGEMENT.UPDATE_SUCCESS")
                : this.translate.instant("PROVINCIAL_MANAGEMENT.ADD_SUCCESS")
            );
            this.activeModal.close();
            this.handleClose();
          } else {
  ...
  validateFile(event: any) {
    ...
    let fileExtension: string = file.name.substring(
    ...
    if (file.size > MAX_FILE_SIZE_TEMPLATE) {
      this.toastService.error(
        this.translate.instant("VALIDATION.FILE_MAX_SIZE", { 0: 10 })
      );
      return;
    }
    // // check up trung file
