...
export class ListBtsComponent implements OnInit, AfterViewInit {
  ...
  listApprovedStationStatusToUpdate = [];
  listYesOrNoToUpdate = [];
  ...
  isShowApproveStation = new BehaviorSubject<boolean>(false);
  records = [
    { id: 1, value: 10 },
    { id: 2, value: 15 },
    { id: 3, value: 20 },
  ...
  ngOnInit(): void {
    this.getComboBoxDataBTSStation().subscribe(res => {
      ...
      this.listApprovedStationStatusToUpdate = this.btsManagementService.listApprovedStatus.value;
      this.eSearch();
    });
  }
  ...
  ngAfterViewInit(): void {
    ...
    this.searchForm.get('provinceSearch').valueChanges.subscribe(value => {
      ...
      if (value == '' || value == undefined || value == null) {
        this.btsManagementService.listDistrict.next([]);
        return;
      }
  ...
  startEdit(item): void {
    ...
    if (item.provinceCode) {
      this.getListDistrict(item.provinceCode).subscribe(res => {
        ...
        // let districtObj = item.listDistrictToUpdate.find(dist => dist.distId == item.district);
        // item.districtStr = districtObj.distId;
      });
    }
  }
    ...
    this.conditionSearch().subscribe(res => {
      if (res.errorCode == '0') {
        ...
        this.listStations.value.map(item => {
          ...
          if(item.approvedStatus || item.approvedStatus === 0){
            // let approveObj = this.btsManagementService.listApprovedStatus.value.find(type => type.value == item.approvedStatus);
            item.approvedStatusStr = item.approvedStatus.toString();
          }
  ...
  conditionSearch() {
    const requestTarget = {
      ...
      btsStationDTO: {
        ...
        hasCRFile: parseInt(this.btsManagementService.hasCRFile.value),
        hasContractFile: parseInt(this.btsManagementService.hasContractFile.value)
      }
    };
  ...
  setApproveStatus(event , row) {
    row.approvedStatus = event.target.value
  }
