...
import {Component, ContentChildren, ElementRef, EventEmitter, HostBinding, HostListener, Input, Output, QueryList, Self, ViewChild} from '@angular/core';
import {coerceBooleanProperty} from './helper';
import {coerceNumberProperty} from './helper';
import {CustomDropzoneService, RejectedFile} from './custom-dropzone.service';
import {CustomDropzonePreviewComponent} from './custom-dropzone-preview/custom-dropzone-preview.component';

export interface CustomDropzoneChangeEvent {
  source: CustomDropzoneComponent;
  addedFiles: File[];
...
@Component({
  ...
  providers: [CustomDropzoneService]
})
export class CustomDropzoneComponent {

  constructor(
    @Self() private service: CustomDropzoneService
  ) {}

  /** A list of the content-projected preview children. */
  @ContentChildren(CustomDropzonePreviewComponent, { descendants: true })
  _previewChildren: QueryList<CustomDropzonePreviewComponent>;
  ...
  @Output() readonly change = new EventEmitter<CustomDropzoneChangeEvent>();
  ...
  @Input()
  @HostBinding('class.custom-dz-disabled')
  ...
  @HostBinding('class.custom-dz-hovered')
  _isHovered = false;
